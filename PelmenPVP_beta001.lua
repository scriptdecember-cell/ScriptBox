...
local CFG_FILE="PelmenPVP_b001_cfg.json"
local DEFAULT={
...
if game.CoreGui:FindFirstChild("PelmenB001") then game.CoreGui.PelmenB001:Destroy() end
local GUI=Instance.new("ScreenGui")
GUI.Name="PelmenB001" GUI.ResetOnSpawn=false
...
local function buildLoadScreen()
    ...
    LImg.Image="rbxassetid://101926931672087"
    LImg.Size=UDim2.new(0,S(128),0,S(128)) LImg.AnchorPoint=Vector2.new(0.5,0)
    LImg.Position=UDim2.new(0.5,0,0,0) LImg.BackgroundTransparency=1 LImg.ZIndex=10001 LImg.ImageTransparency=1
    uiCorner(LImg,S(28))

    local LFb=Instance.new("TextLabel",LC)
    LFb.Text="🥟" LFb.Size=UDim2.new(0,S(128),0,S(128)) LFb.AnchorPoint=Vector2.new(0.5,0)
    LFb.Position=UDim2.new(0.5,0,0,0) LFb.BackgroundTransparency=1 LFb.TextScaled=true LFb.ZIndex=10000
    LImg:GetPropertyChangedSignal("IsLoaded"):Connect(function() if LImg.IsLoaded then LFb.Visible=false end end)
    ...
    LV.Text="Beta 0.0.1  ·  "..(IsMobile and "📱 Mobile" or "🖥️ PC").."  ·  Safe"
    ...
    LS.Text="Инициализация..." LS.Size=UDim2.new(1,0,0,S(16)) LS.Position=UDim2.new(0,0,0,S(212))
    ...
    LSub.Text="Delta · Arceus X · Fluxus · Synapse  |  Beta 0.0.1"
    ...
    return LF,LImg,LT,LV,LS,BF,LSub,Glow
end

local LF,LImg,LT,LV,LS,BF,LSub,LGlow=buildLoadScreen()

local function finishLoad()
    animLoad(BF,LS,1.0,"Готово ✓")
    task.wait(0.7)
    Cfg.AntiAC.On=false StopAntiAC()
...
task.spawn(function()
    Tw(LImg, {ImageTransparency=0},0.7,Enum.EasingStyle.Quart)
    ...
    animLoad(BF,LS,0.1,"Ожидание игрока...") task.wait(0.05)
    repeat task.wait(0.15) until LP.Character and LP.Character:FindFirstChildOfClass("Humanoid")
    animLoad(BF,LS,0.28,"Загрузка мира...") task.wait(0.05)
    repeat task.wait(0.2) until game:IsLoaded()
    animLoad(BF,LS,0.45,"Инициализация UI...") task.wait(0.05)
    animLoad(BF,LS,0.60,"Fly система...") task.wait(0.05)
    animLoad(BF,LS,0.74,"Fast Collect...") task.wait(0.05)
    animLoad(BF,LS,0.86,"Anti-Cheat...") task.wait(0.05)
    animLoad(BF,LS,0.95,"Финальная проверка...") task.wait(0.15)
    finishLoad()
end)
...
HFallback.Text="🥟" HFallback.Size=UDim2.new(1,0,1,0) HFallback.BackgroundTransparency=1 HFallback.TextScaled=true HFallback.ZIndex=6
...
VerL.Text="Beta 0.0.1 · "..(IsMobile and "📱 Mobile" or "🖥️ PC")
...
MinBtn.Text="−" MinBtn.Size=UDim2.new(0,S(30),0,S(30))
...
local function ActionBtn(label,icon,color,desc,onClick)
    ...
    local RunBtn=Instance.new("TextButton",Card) RunBtn.Text="▶ Run"
...
if IsMobile then
    ...
    dBtn("▲",cx,0,                       function(v) mobFlyCtrl.f=v end)
    dBtn("▼",cx,DPAD_SZ-BTN,             function(v) mobFlyCtrl.b=v end)
    dBtn("◀",0,(DPAD_SZ-BTN)/2,          function(v) mobFlyCtrl.l=v end)
    dBtn("▶",DPAD_SZ-BTN,(DPAD_SZ-BTN)/2,function(v) mobFlyCtrl.r=v end)
    ...
    vBtn("▲UP",function(v) mobFlyCtrl.u=v end)
    vBtn("▼DN",function(v) mobFlyCtrl.d=v end)
end
...
RunService.Heartbeat:Connect(function()
    ...
    lastCollect=tick() pcall(FireCollectE)
end)

local _kickHooked=false
local function HookAntiKick()
    if _kickHooked then return end _kickHooked=true
    pcall(function()
        ...
        local function newNc(self,...)
            ...
            if method=="Kick" and self==LP then
                Notify("🛡️ KICK BLOCKED","Игра пыталась кикнуть!",Color3.new(1,1,1),5)
                return
            end
            return oldNc(self,...)
        end
...
local function HookRemotes()
    pcall(function()
        ...
        pcall(function() if setreadonly then setreadonly(mt,false) end end)
        local function isSusp(name)
            local n=tostring(name):lower()
            return n:find("anticheat") or n:find("anti_cheat") or n:find("detect")
                or n:find("cheat") or n:find("verify") or n:find("report")
                or n:find("ban") or n:find("flag") or n:find("kick")
        end
        local function newNc(self,...)
            local method=(getnamecallmethod and getnamecallmethod()) or ""
            if method=="Kick" and self==LP then
                Notify("🛡️ KICK","Заблокирован",Color3.new(1,1,1),4) return
            end
            if (method=="FireServer" or method=="InvokeServer") then
                local ok,isRE=pcall(function() return self:IsA("RemoteEvent") or self:IsA("RemoteFunction") end)
                if ok and isRE then
                    if isSusp(self.Name) or isSusp(self.Parent and self.Parent.Name or "") then
                        _blockedCount=_blockedCount+1 return
                    end
                end
            end
            return oldNc(self,...)
...
local function StartAntiAC(h)
    ...
    local function forceWS()
        ...
        local t=Cfg.Speed.On and Cfg.Speed.Value or 16
        if h and h.WalkSpeed~=t then for _=1,12 do pcall(function() h.WalkSpeed=t end) end end
    end
    local function forceJP()
        if not Cfg.AntiAC.On then return end
        local t=Cfg.InfJump.On and Cfg.InfJump.Power or 50
        if h and h.JumpPower~=t then for _=1,12 do pcall(function() h.JumpPower=t end) end end
    end
    local c1=h:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
        task.defer(forceWS) task.delay(0.01,forceWS) task.delay(0.05,forceWS)
    end)
    table.insert(_acConns,c1)
    local c2=h:GetPropertyChangedSignal("JumpPower"):Connect(function()
        task.defer(forceJP) task.delay(0.01,forceJP) task.delay(0.05,forceJP)
    end)
    table.insert(_acConns,c2)
    local c3=h:GetPropertyChangedSignal("Health"):Connect(function()
        if Cfg.AntiAC.On and Cfg.AntiRag.On and h.Health<=0 then
            task.defer(function() pcall(function() h.Health=h.MaxHealth end) end)
        end
    end)
    table.insert(_acConns,c3)
    if setmetatable then
        pcall(function()
            local orig=(getrawmetatable and getrawmetatable(h)) or getmetatable(h) or {}
            _acMeta[h]=orig
            local mt={} for k,v in pairs(orig) do mt[k]=v end
            local oldNI=mt.__newindex
            mt.__newindex=function(self,key,val)
                if Cfg.AntiAC.On then
                    if key=="WalkSpeed" and Cfg.Speed.On then
                        local v2=Cfg.Speed.Value
                        if oldNI then oldNI(self,key,v2) else rawset(self,key,v2) end return
                    end
                    if key=="JumpPower" and Cfg.InfJump.On then
                        local v2=Cfg.InfJump.Power
                        if oldNI then oldNI(self,key,v2) else rawset(self,key,v2) end return
                    end
                    if key=="PlatformStand" and val==true then return end
                    if key=="Sit" and val==true then return end
                end
                if oldNI then oldNI(self,key,val) else rawset(self,key,val) end
            end
            pcall(function()
                if setreadonly then setreadonly(h,false) end
                setmetatable(h,mt)
            end)
        end)
    end
    ...
    task.spawn(function()
        while Cfg.AntiAC.On and h and h.Parent do
            for _,svc in pairs({game:GetService("ReplicatedStorage"),Char()}) do
                if svc then for _,scr in pairs(svc:GetDescendants()) do
                    if (scr:IsA("LocalScript") or scr:IsA("Script")) and not scr:IsA("CoreScript") then
                        local n=scr.Name:lower()
                        if n:find("anti") or n:find("detect") or n:find("cheat") then
    ...
    Notify("🔮 AntiAC","5 слоёв · Kick·Remote·Meta·Loop·Script",Color3.new(0.9,0.9,0.9),4)
end
local TABS={{name="Combat",icon="⚔️"},{name="Move",icon="🏃"},{name="Fly",icon="✈️"},{name="Server",icon="🌐"},{name="Config",icon="⚙️"}}
...
local function SwitchTab(name)
    ...
    if name=="Combat" then
        Section("Fling",T.Red)
        Toggle("Mini Fling","💥","G = кинуть ближайшего",T.Red,function() return Cfg.Fling.On end,function(v) Cfg.Fling.On=v end,function(v) Notify("💥 Fling",v and "ON" or "OFF",T.Red,2) end)
        Slider("Fling Power","🚀",T.Red,50,800,function() return Cfg.Fling.Power end,function(v) Cfg.Fling.Power=v end," pw")
        Slider("Fling Range","📡",T.Red,5,60,function() return Cfg.Fling.Range end,function(v) Cfg.Fling.Range=v end," st")
        Section("Auto Spam",T.Acc)
        Toggle("Auto Spam","⚡","Авто-стрельба / клик",T.Acc,function() return Cfg.Spam.On end,function(v) Cfg.Spam.On=v end,function(v) Notify("⚡ Spam",v and "ON" or "OFF",T.Acc,2) end)
        Slider("Spam Delay","⏱",T.Acc,1,50,function() return Cfg.Spam.Delay end,function(v) Cfg.Spam.Delay=v end," ms")
        Section("Fast Collect",T.Green)
        Toggle("Fast Collect E","🌿","Авто-сбор: E + Prompts",T.Green,function() return Cfg.FastCollect.On end,function(v) Cfg.FastCollect.On=v end,function(v) Notify("🌿 Collect",v and "ON" or "OFF",T.Green,2) end)
        Slider("Collect Rate","⏩",T.Green,1,20,function() return Cfg.FastCollect.Rate end,function(v) Cfg.FastCollect.Rate=v end," (1=fast)")

    elseif name=="Move" then
        Section("Speed",T.Acc)
        Toggle("Speed Hack","💨","Кастомная скорость",T.Acc,function() return Cfg.Speed.On end,
            function(v) Cfg.Speed.On=v local h=Hum() if h then h.WalkSpeed=v and Cfg.Speed.Value or 16 end end,
            function(v) Notify("💨 Speed",v and "ON "..Cfg.Speed.Value.."WS" or "OFF",T.Acc,2) end)
        Slider("Speed Value","🔢",T.Acc,16,500,function() return Cfg.Speed.Value end,
            function(v) Cfg.Speed.Value=v if Cfg.Speed.On then local h=Hum() if h then h.WalkSpeed=v end end end," ws")
        Section("Jump",T.Acc2)
        Toggle("Infinity Jump","🚀","Прыжок без лимита",T.Acc2,function() return Cfg.InfJump.On end,function(v) Cfg.InfJump.On=v end,function(v) Notify("🚀 InfJump",v and "ON" or "OFF",T.Acc2,2) end)
        Slider("Jump Power","⬆️",T.Acc2,50,400,function() return Cfg.InfJump.Power end,function(v) Cfg.InfJump.Power=v end," jp")
        Section("Защита",T.Acc)
        Toggle("Shift Lock","🔒","Поворот за камерой",T.Acc,function() return Cfg.ShiftLock.On end,function(v) Cfg.ShiftLock.On=v end,function(v) Notify("🔒 ShiftLock",v and "ON" or "OFF",T.Acc,2) end)
        Toggle("Anti Ragdoll","🛡️","Отключает суставы",T.Green,function() return Cfg.AntiRag.On end,function(v) Cfg.AntiRag.On=v end,function(v) Notify("🛡️ AntiRag",v and "ON" or "OFF",T.Green,2) end)
        Toggle("Anti Anti-Cheat","🔮","Signal + Meta + Loop (x3)",T.Acc2,function() return Cfg.AntiAC.On end,
            function(v) Cfg.AntiAC.On=v if v then local h=Hum() if h then StartAntiAC(h) end else StopAntiAC() end end,
            function(v) Notify("🔮 AntiAC",v and "3 слоя защиты" or "OFF",T.Acc2,2) end)

    elseif name=="Fly" then
        Section("✈️ Fly System",T.Acc)
        ...
        local lines=IsMobile and {
            "📱 D-Pad (справа) — вперёд / назад / лево / право",
            "▲UP / ▼DN — высота",
            "🔘 Fly кнопка внизу — вкл/выкл",
        } or {
            "⌨️ WASD — направление  |  F — вкл/выкл",
            "Space — вверх  |  C / LCtrl — вниз",
            "Fly Speed × 10 = studs/sec",
        }
        ...
        Toggle("Fly","✈️","Полёт R6 + R15",T.Acc,function() return flyActive end,
            function(v) if v and not flyActive then StartFly() elseif not v and flyActive then StopFly() end end,
            function(v) Cfg.Fly.On=v ScheduleSave() Notify("✈️ Fly",v and "Включён!" or "Выключен",T.Acc,2) end)
        Slider("Fly Speed","🏎️",T.Acc,1,10,function() return Cfg.Fly.Speed end,function(v) Cfg.Fly.Speed=v end," x")

    elseif name=="Server" then
        Section("Server Tools",T.Acc)
        ActionBtn("Rejoin","🔄",T.Acc,"Переподключиться",function()
            Notify("🔄 Rejoin","Переподключаемся...",T.Acc,2)
            task.delay(1,function() pcall(function() TeleportService:Teleport(game.PlaceId,LP) end) end)
        end)
        ActionBtn("Server Hop","🌐",T.Acc2,"Найти новый сервер",function()
            Notify("🌐 ServerHop","Ищем сервер...",T.Acc2,3)
            task.spawn(function()
                ...
                if ok2 and servers and servers.data then
                    for _,s in ipairs(servers.data) do
                        if s.id~=game.JobId and s.playing<s.maxPlayers then
                            Notify("🌐 Прыгаем!","Сервер найден",T.Acc2,2) task.wait(0.5)
                            pcall(function() TeleportService:TeleportToPlaceInstance(game.PlaceId,s.id,LP) end) return
                        end
                    end
                end
                Notify("🌐 Fallback","Перезаход",T.Acc2,2) task.wait(0.5)
                pcall(function() TeleportService:Teleport(game.PlaceId,LP) end)
            end)
        end)
        Section("Lag",T.Red)
        Toggle("Server Lag","⚡","Спам RemoteEvents",T.Red,function() return Cfg.ServLag.On end,function(v) Cfg.ServLag.On=v end,function(v) Notify("⚡ ServLag",v and "ЛАГАЕМ!" or "Стоп",T.Red,2) end)
        Slider("Lag Threads","🧵",T.Red,10,200,function() return Cfg.ServLag.Threads end,function(v) Cfg.ServLag.Threads=v end," thr")

    elseif name=="Config" then
        Section("💾 Config",T.Yellow)
        ...
        local function cfgStatus()
            if _S.readfile then local ok2,raw=sCall(_S.readfile,CFG_FILE) if ok2 and raw and #raw>2 then return "✅ Найден ("..#raw.." байт)" end end
            return _S.writefile and "⚠️ Не создан" or "❌ ФС не поддерживается"
        end
        local FL=Instance.new("TextLabel",IC) FL.Text="📂 "..CFG_FILE FL.Size=UDim2.new(1,-S(18),0,S(20)) FL.Position=UDim2.new(0,S(12),0,S(6))
        ...
        local AL=Instance.new("TextLabel",IC) AL.Text="⚡ Авто-сохранение через 1.5с" AL.Size=UDim2.new(1,-S(18),0,S(13)) AL.Position=UDim2.new(0,S(12),0,S(46))
        ...
        ActionBtn("Save Now","💾",T.Yellow,"Принудительное сохранение",function() SaveCfg(Cfg) SL.Text=cfgStatus() Notify("💾 Сохранено","CFG обновлён",T.Yellow,2) end)
        ActionBtn("Reset Defaults","🔄",T.Red,"Сброс всех настроек",function()
            Cfg=DeepCopy(DEFAULT) SaveCfg(Cfg) SL.Text=cfgStatus() if flyActive then StopFly() end Notify("🔄 Сброс","По умолчанию",T.Red,2)
        end)
        Section("Экран",T.Acc)
        ...
        SIL.Text=string.format("📐 %dx%d  ·  Scale %.2f  ·  %s",math.floor(vx),math.floor(vy),scaleVal(),IsMobile and "Mobile" or "PC")
        ...
        local SIL2=Instance.new("TextLabel",SIC) SIL2.Text="Beta 0.0.1  ·  Executor safe  ·  Anti-AC: 3 layers"
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    TabBar.Visible=not minimized Scroll.Visible=not minimized MinBtn.Text=minimized and "+" or "−"
end)
...
if IsMobile then
    ...
    MobBtn("✈️Fly",function() if not flyActive then StartFly() else StopFly() end Notify("✈️",flyActive and "ON" or "OFF",T.Acc,1.5) end)
    MobBtn("💨Spd",function() Cfg.Speed.On=not Cfg.Speed.On ScheduleSave() local h=Hum() if h then h.WalkSpeed=Cfg.Speed.On and Cfg.Speed.Value or 16 end Notify("💨",Cfg.Speed.On and "ON" or "OFF",T.Acc,1.5) end)
    MobBtn("🚀Jmp",function() Cfg.InfJump.On=not Cfg.InfJump.On ScheduleSave() Notify("🚀",Cfg.InfJump.On and "ON" or "OFF",T.Acc2,1.5) end)
    MobBtn("🌿Clc",function() Cfg.FastCollect.On=not Cfg.FastCollect.On ScheduleSave() Notify("🌿",Cfg.FastCollect.On and "ON" or "OFF",T.Green,1.5) end)
    MobBtn("💥Flg",function()
        if not Cfg.Fling.On then Notify("💥","Включи в Combat!",T.Red,2) return end
    ...
    MobBtn("🌐Hop",function() Notify("🌐","Переход...",T.Acc2,2) task.delay(1,function() pcall(function() TeleportService:Teleport(game.PlaceId,LP) end) end) end)
end
...
UIS.InputBegan:Connect(function(input,gpe)
    ...
    if input.KeyCode==Enum.KeyCode.F then
        ...
        Notify("✈️ Fly",flyActive and "Включён!" or "Выключен",T.Acc,2)
    end
...
local function ApplyAll(char,isRespawn)
    ...
    if isRespawn then
        ...
        if #feats>0 then Notify("🔄 Reload",table.concat(feats," "),T.Acc,3) end
    end
end
LP.CharacterAdded:Connect(function(char) StopAntiAC() ApplyAll(char,true) end)
...
task.delay(3.2,function()
    Notify("🥟 Pelmen PVP","Beta 0.0.1 · "..(IsMobile and "📱" or "🖥️").." · D-Pad · AntiAC×3 · Rainbow",T.Acc,5)
end)
task.delay(4.8,function()
    local on={}
    if Cfg.Speed.On then table.insert(on,"💨"..Cfg.Speed.Value) end
    if Cfg.InfJump.On then table.insert(on,"🚀") end
    if Cfg.FastCollect.On then table.insert(on,"🌿") end
    if Cfg.Spam.On then table.insert(on,"⚡") end
    if Cfg.AntiRag.On then table.insert(on,"🛡️") end
    if Cfg.AntiAC.On then table.insert(on,"🔮") end
    if #on>0 then Notify("📂 CFG",table.concat(on," "),T.Yellow,5) end
end)

print("╔══════════════════════════════════════════════╗")
print("║  🥟 PELMEN PVP  Beta 0.0.1  Rainbow UI     ║")
print("║  Scale: "..string.format("%.2f",math.min(math.clamp(SW/1280,0.45,2.0),math.clamp(SH/720,0.45,2.0))).."  |  "..math.floor(SW).."x"..math.floor(SH).."  |  "..(IsMobile and "Mobile" or "PC   ").."    ║")
print("╚══════════════════════════════════════════════╝")
